...
uint_fast32_t crc32_for_byte(uint32_t r) {
  for(int j = 0; j < 8; ++j)
    r = (r & 1? 0: (uint_fast32_t)0xEDB88320L) ^ r >> 1;
  return r ^ (uint_fast32_t)0xFF000000L;
}

uint32_t crc32(const uint8_t *data, size_t n_bytes) {
  static uint32_t table[0x100];
  uint_fast32_t crc = 0x00;
  if(!*table){
